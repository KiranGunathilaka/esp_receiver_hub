...
static int peer_find(const uint8_t *mac) {
    for (int i=0;i<24;i++) if (peers[i].in_use && mac_equal(peers[i].mac,mac)) return i;
    return -1;
}
static int peer_get_or_add(const uint8_t *mac) {
    int i = peer_find(mac);
    if (i>=0) return i;
    for (int k=0;k<24;k++) if (!peers[k].in_use) {
...
{
    if (!info || len < (int)sizeof(rfid_msg_t)) return;
    ...
    if (idx >= 0) peers[idx].first_char = (uint8_t)plain[0];
...
static void espnow_sent_cb(const uint8_t *mac, esp_now_send_status_t st) {
    (void)mac; (void)st; // optional logging
}
...
static void uart_tx_task(void *arg) {
    ...
    for (;;) {
        ...
        char macs[18]; mac_to_str(front.mac, macs);
...
static void uart_rx_task(void *arg) {
    ...
    for (;;) {
        if (!uart_read_line(line, sizeof(line))) continue;
        ...
        if (strcmp(t->valuestring, "resp") == 0) {
            ...
            if (cJSON_IsNumber(id) && cJSON_IsString(macs) && cJSON_IsNumber(st)) {
                ...
                if (cJSON_IsString(ticket)) strncpy(out.status.ticket_id, ticket->valuestring, sizeof(out.status.ticket_id)-1);
                if (cJSON_IsString(name))   strncpy(out.status.name,      name->valuestring,   sizeof(out.status.name)-1);
